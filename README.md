...
```bash
dotnet new install BongoCatModding.BepInExTemplate
```
...
Once installed, the template will be available as `BongoCat BepInEx Plugin` with an alias `bcmod`.

## Creating a Project

Open a terminal in your BongoCat modding directory, and run:
...
```sh
dotnet new bcmod --name ModName --guid com.github.YourAccount.ModName --ts-team YourThunderstoreTeam
```
...
>
> You can run `dotnet new bcmod --help` to see all available options.
...
```sh
~/Workspace/BongoCat$ dotnet new bcmod --name MyCoolMod --guid com.github.BongoCatModding.MyCoolMod --ts-team BongoCatModding
The template "BongoCat BepInEx Plugin" was created successfully.

~/Workspace/BongoCat$ cd MyCoolMod/
~/Workspace/BongoCat/MyCoolMod$ tree
.
├── CHANGELOG.md
